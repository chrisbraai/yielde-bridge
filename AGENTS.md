...
- `app/` — four rooms (`configure/`, `run/`, `inspect/`, `library/`). Each room has its own layout. The Inspect room is read-only over kernel audit, brain `_inbox/` drafts, capability decisions, and skill telemetry. The Library room is a personal-use surface over every installed skill (`~/.claude/skills/`) and every operator agent (`~/.claude/operator/agents/`) with click-to-copy name pills; reads filesystem on every render.
